...
#Bin
import Articulos.disenoIndustrial, Articulos.init
...
wb = openpyxl.load_workbook('./Input/Base_Grupos.xlsx')
sheet = wb['Sheet1']
...
for q in range(2,total+1):
    ...
    print(my_url)
    index1 = my_url.find("nro=") + 4
    index2 = len(my_url)
    RH = my_url[index1:index2]


    if my_url != '-':
        Articulos.disenoIndustrial.artiextract()
        COD_PRODUCTO = int("".join(str(x) for x in Articulos.disenoIndustrial.contarDiseno))
...
import printcsvdisenoindustrial