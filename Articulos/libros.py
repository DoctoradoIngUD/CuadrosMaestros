def artiextract():
    from main2 import my_url, name, RH, COD_PRODUCTO

    import Articulos.init, bs4, logging, sys, re
    global contarlibro
    ...
    if all != 0:
        ...
        for x in range(0, len(container)):
            ...
            #titulo
            index1 = info_libro.find(' En:')
            index2 = info_libro.find('"', 0, index1 - 1)
            titulo = info_libro[index2 + 1:index1 - 3]
          
            #Lugar
            index1 =  info_libro.find('En:') + 4
            ...
            fecha = info_libro[index2 : index2 + 4]
           
            #Editorial
            index1 = info_libro.find('ed:') + 3
            index2 = info_libro.find('ISBN') - 2
            editorial = info_libro[index1:index2]

            # ISBN
            index1 = info_libro.find("ISBN:") 
            index2 = info_libro.find("\n", index1) - 1
            ISBN = info_libro[index1+6:index2]
       
            #Paginas
            index1 = info_libro.find("pags.") 
            paginas = info_libro[index1 + 6 : index1 + 9]
